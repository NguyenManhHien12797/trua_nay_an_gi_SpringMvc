...
public class CartRepositoryImpl implements ICartRepository {
	...
	public List<Cart> findAllCartByUserIdAndDeleteFlag(Long userId) {
		...
		List<Cart> carts = session
				.createQuery("FROM Cart c Where c.deleteFlag = false and c.user_id=" + userId, Cart.class)
				.getResultList();
		return carts;
	}
	...
	public Cart findCartByProductIdAndUserId(Long product_id, Long user_id) {
		Session session = this.sessionFactory.getCurrentSession();
		Query query = session.createQuery("Select c FROM Cart c WHERE c.deleteFlag = false and c.product =" + product_id
				+ " and " + " c.user_id =" + user_id, Cart.class);
		try {
